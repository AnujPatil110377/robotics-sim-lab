...
function resolveDockerClient() {
  ...
  if (process.platform === 'win32') {
    const pipes = ['docker_engine', 'dockerDesktopEngine', 'dockerDesktopLinuxEngine'];
    for (const pipeName of pipes) {
      const winPipePath = `\\\\.\\pipe\\${pipeName}`;
      if (fs.existsSync(winPipePath)) {
        return new Docker({ socketPath: `//./pipe/${pipeName}` });
      }
    }
  }
...
const docker = resolveDockerClient();
